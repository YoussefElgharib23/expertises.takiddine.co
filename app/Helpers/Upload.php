...
class Upload {
    ...
    public function save(){
            ...
            if ($handle->uploaded) {
                $nameimmg = $this->text_random();
                $handle->file_new_name_body   = $nameimmg;
                $handle->image_text = 'Renov Expertise '. \Carbon\Carbon::Now();
                $handle->image_text_size = 75;
                $handle->image_text_position  = 'RB';
                $handle->image_text_color      = '#FFFFFF';
                $handle->image_text_size       = 80;
                $handle->image_text_font       =BASEPATH.'/public/Raleway-Medium.ttf';

               $handle->process($this->path);
                  if ($handle->processed) {
                    $myfile = $nameimmg.'.'.$handle->file_dst_name_ext;
                    $handle->clean();
                    return $myfile;
                  } else {
